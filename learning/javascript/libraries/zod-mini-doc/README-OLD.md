...
const scoreSchema = z.number().int().gte(0, "Pontuação deve ser positiva").lte(100, "Pontuação máxima é 100");
```
...
const passwordSchema = z
  ...
  .refine((password) => /[^A-Za-z0-9]/.test(password), "Senha deve conter pelo menos um caractere especial");
...
const pointWithMetadataSchema = z.tuple([z.number(), z.number()]).rest(z.string()); // pode ter mais strings depois dos dois números
...
const personUnion = z.discriminatedUnion("type", [userShape, adminShape]);
...
const personWithEmail = z.intersection(basePerson, withEmail);
...
const formSchema = z
  .object({
    ...
    newsletter: z.string().transform((val) => val === "yes"),
  })
  .transform((data) => ({
    ...data,
...
const validationPlugins = {
  BR: (schema: typeof baseProductSchema) =>
    schema.extend({
      price: z
        .number()
        .positive()
        .refine((price) => price > 0.1, { message: "Preço mínimo deve ser maior que R$ 0,10" }),
      tax: z.number().min(0).max(0.4),
    }),

  US: (schema: typeof baseProductSchema) =>
    schema.extend({
      price: z
        .number()
        .positive()
        .refine((price) => price > 0.01, { message: "Preço mínimo deve ser maior que $0.01" }),
  ...
  EU: (schema: typeof baseProductSchema) =>
    schema.extend({
      price: z.number().positive(),
...
class SchemaBuilder<T extends z.ZodTypeAny> {
  ...
  withEmail(options?: { required?: boolean }) {
    ...
    const finalSchema = options?.required === false ? emailSchema.optional() : emailSchema;
  ...
  private extend<K extends string, V extends z.ZodTypeAny>(fields: Record<K, V>) {
...
const userSchema = createSchema().withEmail().withPassword(10).withAge({ min: 18 }).build();
...
const customerSchema = createSchema().withEmail().withAddress().build();
...
const validatedUserSchema = rawUserDataSchema
  .refine((data) => data.email.includes("@"), { path: ["email"], message: "Email deve conter @" })
  .refine((data) => /^\d{4}-\d{2}-\d{2}$/.test(data.birthDate), {
    path: ["birthDate"],
    message: "Data de nascimento deve estar no formato YYYY-MM-DD",
  });

// Camada 2: Transformação para Modelo de Domínio
const domainUserSchema = validatedUserSchema.transform((data) => ({
  ...data,
...
const businessRulesUserSchema = domainUserSchema
  .refine((data) => data.age >= 18, { message: "Usuário deve ter pelo menos 18 anos" })
  .refine(
    (data) => {
...
const betterFormSchema = z.object({
  ...
  notes: z.preprocess((val) => (typeof val === "string" && val.trim() === "" ? undefined : val), z.string().optional()),
});
...
function validateRequest<T extends z.ZodTypeAny>(schema: T, source: "body" | "query" | "params" = "body") {
...
const userQuerySchema = z.object({
  ...
  active: z.preprocess((val) => (val === "true" ? true : val === "false" ? false : val), z.boolean().optional()),
  page: z.preprocess(
...
app.get("/users/:id", validateRequest(userIdSchema, "params"), (req, res) => {
  const { id } = req.params;
  // Lógica para buscar usuário por ID...
  res.json({ status: "success", data: { id, name: "Exemplo" } });
});

app.get("/users", validateRequest(userQuerySchema, "query"), (req, res) => {
...
const taskSchema = baseTaskSchema
  .pipe(withId(baseTaskSchema))
  .pipe(withTimestamps(withId(baseTaskSchema)))
  .pipe(withSoftDelete(withTimestamps(withId(baseTaskSchema))))
  .pipe(withVersioning(withSoftDelete(withTimestamps(withId(baseTaskSchema)))));
