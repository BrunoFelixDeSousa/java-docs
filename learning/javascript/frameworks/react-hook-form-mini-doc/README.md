...
import { FormControl, FormLabel, FormErrorMessage, Input, Button, VStack, Box } from "@chakra-ui/react";
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <VStack spacing={4} align="flex-start">
          <FormControl isInvalid={!!errors.name}>
            ...
            <Input id="name" placeholder="Seu nome" {...register("name", { required: "Nome é obrigatório" })} />
          ...
          <FormControl isInvalid={!!errors.email}>
            ...
            <Controller
              ...
              render={({ field }) => <Input id="email" placeholder="seu-email@exemplo.com" {...field} />}
            />
      ...
      <div>
        ...
        {fields.map((field, index) => (
          <div key={field.id}>
            ...
            {errors.phoneNumbers?.[index]?.number && <span>{errors.phoneNumbers[index]?.number?.message}</span>}
          </div>
        ))}

        <button type="button" onClick={() => append({ type: "home", number: "" })}>
          Adicionar Telefone
        </button>
      </div>
...
const Step1 = () => {
  ...
  return (
    <div>
      ...
      <div>
        <label htmlFor="firstName">Primeiro Nome</label>
        <input id="firstName" {...register("firstName", { required: "Primeiro nome é obrigatório" })} />
        {errors.firstName && <span>{errors.firstName.message}</span>}
      </div>

      <div>
        <label htmlFor="lastName">Sobrenome</label>
        <input id="lastName" {...register("lastName", { required: "Sobrenome é obrigatório" })} />
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mb-4">
          ...
          {errors.variants?.message && (
            <span className="text-red-500 text-sm block mb-2">{errors.variants.message}</span>
          )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  {errors.variants?.[index]?.name && (
                    <span className="text-red-500 text-sm mt-1">{errors.variants[index]?.name?.message}</span>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.variants?.[index]?.price && (
                    <span className="text-red-500 text-sm mt-1">{errors.variants[index]?.price?.message}</span>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.variants?.[index]?.sku && (
                    <span className="text-red-500 text-sm mt-1">{errors.variants[index]?.sku?.message}</span>
                  )}
                </div>
              </div>
            </div>
...
const UserForm = () => {
  ...
  return <form onSubmit={handleSubmit(onSubmit)}>{/* Campos do formulário */}</form>;
};
```
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      {submitStatus === "success" && (
        <div className="p-3 bg-green-100 text-green-700 rounded mb-4">Formulário enviado com sucesso!</div>
      )}
